        ...
        for seq in data:
            ...
            for d in seq:
                ...
                if d < self.opt["source_item_num"]:
                    if source is not -1:
                        if d in real_adj[source]:
                            ...
                            VV_edges_single.append([d, source])
                    source = d
                    ...
                    if target is not -1:
                        if d in real_adj[target]:
                            ...
                            VV_edges_single.append([d, target])
                    target = d
                ...
                if pre is not -1:
                    ...
                    VV_edges.append([d, pre])
                pre=d

        def deduplicate(edges):
            df = pd.DataFrame(edges, columns=['row', 'col'])
            df.drop_duplicates(inplace=True)
            return df.values.tolist()

        VV_edges = np.array(deduplicate(VV_edges))
        VV_edges_single = np.array(deduplicate(VV_edges_single))
